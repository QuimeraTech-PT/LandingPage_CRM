...
export function ActivityTimeline({ logs, className }: ActivityTimelineProps) {
  ...
  return (
    <div
      className={cn(
        "relative space-y-6 before:absolute before:inset-0 before:ml-5 before:-translate-x-px before:h-full before:w-0.5 before:bg-linear-to-b before:from-primary/20 before:via-primary/10 before:to-transparent",
        className,
      )}
    >
      {logs.map((log, idx) => {
              ...
              <div className="bg-card/30 rounded-xl p-3 border border-white/5 group-hover:border-primary/20 transition-all">
                <p className="text-xs text-muted-foreground leading-relaxed">
                  ...
                  {log.action === "update" && log.old_value && log.new_value ? (
                    <span>
                      Alterou de{" "}
                      <span className="text-red-400/80 italic">{formatValue(log.old_value)}</span>{" "}
                      para{" "}
