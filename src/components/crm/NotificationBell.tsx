  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end" className="w-80 bg-card border-white/10 p-0 shadow-2xl">
            ...
            notifications.map((n: any) => (
              ...
              >
                <div className="flex items-center gap-2 w-full">
                  {getIcon(n.type)}
                  <span className={cn("text-xs font-bold flex-1 truncate", !n.read ? "text-foreground" : "text-muted-foreground")}>
                    {n.title}
                  </span>
                  {!n.read && <div className="h-2 w-2 rounded-full bg-primary shadow-lg shadow-primary/40" />}
                </div>
        ...
        <DropdownMenuSeparator className="bg-white/5 m-0" />
        <Button variant="ghost" className="w-full text-xs text-muted-foreground rounded-none hover:bg-white/5 py-3">
          Ver Todas
        </Button>
      </DropdownMenuContent>
    </DropdownMenu>
